...
def _load_config():
    ...
    config = {
        ...
        "upload_api_token": "",
        # ===== 安全默认：尽量减少敏感信息落盘 =====
        # 是否在 output_file 中保存 ChatGPT 密码/邮箱密码（不推荐）
        ...
        # 上传到管理 API 时是否校验证书（HTTPS 有效；默认 True 更安全）
        "upload_api_verify": True,
    }

    # 允许 launcher 通过环境变量指定临时配置文件路径，避免在项目目录常驻敏感 config.json
    config_path = (
        os.environ.get("AIPROXYHUB_REGISTER_CONFIG")
        or os.environ.get("REGISTER_CONFIG_PATH")
    ...
    config["store_passwords"] = os.environ.get("STORE_PASSWORDS", config["store_passwords"])
    config["write_ak_rk"] = os.environ.get("WRITE_AK_RK", config["write_ak_rk"])
    config["keep_token_json"] = os.environ.get("KEEP_TOKEN_JSON", config["keep_token_json"])
    config["keep_token_json_on_fail"] = os.environ.get("KEEP_TOKEN_JSON_ON_FAIL", config["keep_token_json_on_fail"])
    config["upload_api_verify"] = os.environ.get("UPLOAD_API_VERIFY", config["upload_api_verify"])
...
SHOW_SENSITIVE = _as_bool(os.environ.get("AIPROXYHUB_SHOW_SENSITIVE", False))
# 仅用于排障：打印（脱敏后的）HTTP 响应体摘要；默认关闭以降低敏感信息泄露面
DEBUG_HTTP = _as_bool(os.environ.get("AIPROXYHUB_DEBUG_HTTP", False))
...
def _redact_obj(obj):
    """递归脱敏 JSON-like 结构，避免日志中泄露密码/token/code 等。"""
    try:
...
STORE_PASSWORDS = _as_bool(_CONFIG.get("store_passwords", False))
WRITE_AK_RK = _as_bool(_CONFIG.get("write_ak_rk", False))
KEEP_TOKEN_JSON = _as_bool(_CONFIG.get("keep_token_json", False))
KEEP_TOKEN_JSON_ON_FAIL = _as_bool(_CONFIG.get("keep_token_json_on_fail", False))
UPLOAD_API_VERIFY = _as_bool(_CONFIG.get("upload_api_verify", True))
...
def _save_codex_tokens(email: str, tokens: dict):
    ...
    # 明文 token 输出（不推荐，默认关闭）
    if WRITE_AK_RK:
        if access_token and AK_FILE:
            with _file_lock:
                with open(AK_FILE, "a", encoding="utf-8") as f:
                    f.write(f"{access_token}\n")

        if refresh_token and RK_FILE:
            with _file_lock:
    ...
    upload_ok = None
    if UPLOAD_API_URL:
        upload_ok = _upload_token_json(token_path)
    else:
        # 无上传目标时，token_json 是主要产物；为避免误删，强制保留
        upload_ok = None
        if not KEEP_TOKEN_JSON:
            with _print_lock:
                print("  [WARN] 未配置 UPLOAD_API_URL：keep_token_json=false 已忽略（避免丢失 token 输出）")
            return

    # 默认更安全：上传成功后清理 token_json；失败也默认清理（除非显式开启 keep_token_json_on_fail）
    if not KEEP_TOKEN_JSON:
        should_delete = False
        if upload_ok is True:
            should_delete = True
        elif upload_ok is False and not KEEP_TOKEN_JSON_ON_FAIL:
            should_delete = True
...
def _upload_token_json(filepath):
    ...
    try:
        ...
        # 上传目标通常是本机 localhost 管理端口：默认绕过代理，避免代理规则导致本地上传失败
        try:
            host = (urlparse(UPLOAD_API_URL).hostname or "").lower()
        except Exception:
            host = ""
        is_local = host in {"localhost", "127.0.0.1"}
        if DEFAULT_PROXY and not is_local:
...
class ChatGPTRegister:
    ...
    def _log(self, step, method, url, status, body=None):
        ...
        # 默认不打印响应体，避免日志泄露敏感信息；仅在显式开启 DEBUG_HTTP/SHOW_SENSITIVE 时输出
        if body and (DEBUG_HTTP or SHOW_SENSITIVE):
            try:
                payload = body if SHOW_SENSITIVE else _redact_obj(body)
                lines.append(f"{prefix}[Response] {json.dumps(payload, indent=2, ensure_ascii=False)[:1000]}")
            except Exception:
    ...
    def _oauth_allow_redirect_extract_code(self, url: str, referer: str = None):
        ...
        try:
            # 安全默认：不要自动跟随重定向到 redirect_uri（通常是 localhost），避免把授权码泄露给本机恶意服务。
            ...
            for _ in range(10):
                ...
                if code:
                    self._print("[OAuth] redirect 命中响应 URL code")
                    return code
                ...
                next_url = urljoin(cur, loc)
                code = _extract_code_from_url(next_url)
                if code:
                    self._print("[OAuth] redirect 命中 Location code（未发起 localhost 请求）")
                    return code

                # 若下一跳是 localhost/127.0.0.1，则停止跟随，避免意外请求本地回调地址
                try:
                    host = (urlparse(next_url).hostname or "").lower()
                except Exception:
                    host = ""
                if host in {"localhost", "127.0.0.1"}:
                    self._print("[OAuth] redirect 目标为 localhost，已停止跟随（防止泄露）")
                    return None

                cur = next_url
        except Exception as e:
            # 兼容：部分情况下异常信息中会带最终跳转 URL
            maybe_url = re.search(r'(https?://[^\s\'\"]+)', str(e))
            if maybe_url:
                code = _extract_code_from_url(maybe_url.group(1))
                if code:
                    self._print("[OAuth] redirect 从异常提取 code")
                    return code
            self._print(f"[OAuth] redirect 异常: {e}")
    ...
    try:
        ...
        with _file_lock:
            with open(output_file, "a", encoding="utf-8") as out:
                # 更安全的默认：不把密码落盘；保持分隔符格式以兼容旧解析逻辑
                p1 = chatgpt_password if STORE_PASSWORDS else "***"
                p2 = email_pwd if STORE_PASSWORDS else "***"
                out.write(f"{email}----{p1}----{p2}----oauth={'ok' if oauth_ok else 'fail'}\n")
...
def run_batch(total_accounts: int = 3, output_file="registered_accounts.txt",
    ...
    # 容错：允许 total_accounts=0 用于“仅验证环境/脚本链路”或误配置时的安全退出
    try:
    ...
    if ENABLE_OAUTH:
        ...
        keep_note = "保留" if KEEP_TOKEN_JSON else ("失败保留" if KEEP_TOKEN_JSON_ON_FAIL else "自动清理")
        print(f"  Token JSON: {TOKEN_JSON_DIR}/ ({keep_note})")
        print(f"  ak/rk 明文输出: {'开启' if WRITE_AK_RK else '关闭'}")
    print(f"  输出文件: {output_file} ({'含密码' if STORE_PASSWORDS else '不含密码'})")
    print(f"{'#'*60}\n")
