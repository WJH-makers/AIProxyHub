...
if (-not ([string]$env:AIPH_API_KEY).Trim()) {
  ...
  $settingsFile = _FirstExistingFile @(
    (Join-Path $env:LOCALAPPDATA "AIProxyHub\\settings.json"),
  ...
  # 通过 launcher.load_settings() 读取并解密（DPAPI）本地保存的 api_key/admin_api_key
  # 重要：此处不会回显任何 key 明文；仅写入当前 PowerShell 进程环境变量。
  $token = & $py -c @"
import launcher
launcher.SETTINGS_FILE = r'''$settingsFile'''
s = launcher.load_settings()
print((s.get('admin_api_key') or s.get('api_key') or '').strip())
"@
